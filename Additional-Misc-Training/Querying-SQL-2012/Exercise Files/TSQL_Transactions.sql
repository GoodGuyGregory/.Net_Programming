...
-- rolled back if there are any issues with completing the 
-- command.  The main reason is that SQL Server stores the data 
-- on disk and all the data associated with a specific record 
-- may not be stored all in one place.  If we delete only part 
-- of the information for this record, then we corrupt data.
...
-- Nothing is recorded in the transaction log until this 
